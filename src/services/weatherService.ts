import { WeatherData, WeatherAdvisory, DayForecast } from '../types';

// Coordinates dictionary for major Indian districts
export const INDIAN_DISTRICT_COORDS: Record<string, { lat: number; lon: number }> = {
...
export const fetchWeatherData = async (
  ...
  try {
    ...
    const weatherCodeMap: Record<number, { text: string; hindi: string; icon: string }> = {
    ...
    const forecast: DayForecast[] = daily.time.slice(0, 7).map((timeStr: string, idx: number) => {
...
const generateActionableAdvisories = (
  rainProb: number,
  temp: number,
  humidity: number,
  windSpeed: number
...
export const getMockWeatherData = (district: string): WeatherData => {
  return {
    location: district,
    temperature: 31,
    humidity: 65,
