import { CropRecommendation, SoilType, Season, IrrigationType } from '../types';
...
// Master Indian Crops Database with agronomic rules
interface CropMaster {
  ...
  idealNPK: [number, number, number]; // Target N, P, K
  waterNeed: 'Low' | 'Medium' | 'High' | 'Very High';
  waterNeedHindi: string;
  duration: string;
...
const CROP_DATABASE: CropMaster[] = [
  {
    name: 'Wheat',
  ...
  },
  {
    ...
    idealPh: [6.0, 8.0],
    idealNPK: [20, 50, 20], // Legume fixes its own N
    waterNeed: 'Low',
...
export const recommendCrops = (input: RecommendationInput): CropRecommendation[] => {
  const scoredCrops = CROP_DATABASE.map(crop => {
    let score = 50; // Base score

    // 1. Season match (Critical)
  ...
  // Sort descending by suitability score and return top 3
  return scoredCrops
