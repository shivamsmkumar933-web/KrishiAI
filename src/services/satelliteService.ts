import { SatelliteData, StressArea } from '../types';
...
export const getSatelliteCropHealth = async (
  ...
  // 1. Try Live Agromonitoring API call
    ...
    try {
      ...
      if (agroRes.ok) {
        ...
        if (Array.isArray(agroPolygons) && agroPolygons.length > 0) {
          ...
          const polyArea = Math.round(poly.area * 0.247105); // Convert ha to acres

          // Fetch imagery search for this polygon
          const nowSec = Math.floor(Date.now() / 1000);
          const startSec = nowSec - (180 * 24 * 3600); // last 180 days
          ...
          if (imgRes.ok) {
            ...
            if (Array.isArray(images) && images.length > 0) {
              ...
              return {
                ...
                isDemo: false, // 100% Real Agromonitoring Satellite API Data!
  ...
  // 2. Query Microsoft Planetary Computer STAC API Endpoint
  try {
    const delta = 0.05;
    ...
    if (response.ok) {
      ...
      if (features.length > 0) {
        ...
        let healthRating: 'Excellent' | 'Good' | 'Fair' | 'Poor' | 'Severe Stress' = 'Good';
        ...
        return {
          ...
          historicalTrend,
          stressAreas,
          isDemo: false, // 100% Real Microsoft Planetary Computer STAC Data!
...
export const getMockSatelliteData = (
  district: string,
  lat: number,
  lng: number
): SatelliteData => {
